...
static const byte pinSortie1 = 5;
...
static void Entree_Binaire() {
  for (uint8_t IN = 0; IN < NB_PIN_IN; IN++) {
    ...
    VAL_COUR_IN = !digitalRead(PIN_IN[IN]);  //Lecture etat pin inversée
    // Si l'état a changé
    if (VAL_COUR_IN != ETAT_LED_IN[IN]) {
      ...
      log_v("Updating on/off light state to %u", VAL_COUR_IN);
      /* Update on/off light state */
      esp_zb_lock_acquire(portMAX_DELAY);
      err = esp_zb_zcl_set_attribute_val(END_POINT_IN_ID[IN], ESP_ZB_ZCL_CLUSTER_ID_ON_OFF, ESP_ZB_ZCL_CLUSTER_SERVER_ROLE, ESP_ZB_ZCL_ATTR_ON_OFF_ON_OFF_ID, &VAL_COUR_IN, false);
      esp_zb_lock_release();
  ...
  switch (sig_type) {
    ...
    case ESP_ZB_BDB_SIGNAL_DEVICE_REBOOT:
      if (err_status == ESP_OK) {
        ...
        //Démmarage de la lecture et de l'envoi des valeurs
        //xTaskCreate(Capteur_valeur_maj, "sensor_update", 2048, NULL, 10, NULL);

        //xTaskCreate(humi_sensor_value_update, "Maj_des_capteurs", 2048, NULL, 10, NULL);  //Envoie des valeurs d'humidité sur le réseau ZigBee
...
static void esp_zb_task(void *pvParameters) {
  ...
  uint8_t ApplicationVersion = 0x0001;
  uint8_t StackVersion = 0x0002;
  uint8_t HWVersion = 0x0002;
  uint8_t ManufacturerName[] = { 9, 'B', 'i', 'n', 'c', 'h', '&', 'C', 'i', 'e' };
  uint8_t ModelIdentifier[] = { 14, 'T', 'e', 'l', 'e', 'r', 'u', 'p', 't', 'e', 'u', 'r', ' ', 'V', '2' };
  uint8_t DateCode[] = { 8, '2', '0', '2', '6', '0', '5', '0', '7' };
  ...
  esp_zb_endpoint_config_t endpoint_config_1 = {
    ...
    .app_device_id = ESP_ZB_HA_ON_OFF_LIGHT_DEVICE_ID,  //ESP_ZB_HA_ON_OFF_OUTPUT_DEVICE_ID
  };
